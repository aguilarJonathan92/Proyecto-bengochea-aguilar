                            ...
                            <div class="card mb-3 border border-ui-adaptativa shadow-sm overflow-hidden bg-superficie-adaptativa rounded">
                                ...
                                <div class="p-3" style="cursor: pointer;" data-bs-toggle="collapse" data-bs-target="#collapse-{{ $order->id }}" aria-expanded="false" aria-controls="collapse-{{ $order->id }}">
                                    <div class="row align-items-center g-3">
                                        ...
                                        <div class="col-6 col-md-3">
                                            ...
                                            <span class="fw-bold color-dorado-adaptativo">${{ number_format($order->total, 0, ',', '.') }}</span>
                                        </div>

                                        <div class="col-6 col-md-3 text-md-end">
                                            {{-- Manejo dinámico de etiquetas de estado basado en tus strings --}}
                                            @if($order->status === 'en proceso')
                                                <span class="badge bg-warning-subtle text-warning border border-warning px-3 py-2 text-uppercase" style="font-size: 0.7rem;">
                                                    <i class="bi bi-clock-history me-1"></i> Pendiente
                                                </span>
                                            @elseif($order->status === 'completado')
                                                <span class="badge bg-success-subtle text-success border border-success px-3 py-2 text-uppercase" style="font-size: 0.7rem;">
                                                    <i class="bi bi-check-circle me-1"></i> Completado
                                                </span>
                                            @else
                                                <span class="badge bg-danger-subtle text-danger border border-danger px-3 py-2 text-uppercase" style="font-size: 0.7rem;">
                                ...
                                <div id="collapse-{{ $order->id }}" class="collapse {{ $index === 0 ? '' : '' }}" data-bs-parent="#accordionOrders">
                                    <div class="p-3 border-top border-ui-adaptativa bg-light-adaptativa" style="background-color: rgba(0,0,0,0.02);">
                                                    ...
                                                    <div class="card border-0 shadow-sm overflow-hidden item-cart-card">
                                                        <div class="row g-0 align-items-center">
                                                            ...
                                                            <div class="col-9 col-md-10">
                                                                <div class="card-body py-2 d-flex flex-column flex-md-row justify-content-between align-items-md-center">
                                                                    <div>
                                                                        <h6 class="card-title mb-1 fw-bold text-uppercase color-adaptativo" style="font-size: 0.85rem;">
                                                                            {{ $item->product->title }}
                                                                        </h6>
                                                                    ...
                                                                    <div class="mt-2 mt-md-0 text-md-end">
                                                                        <span class="text-muted-adaptativo me-3 small">Cantidad: <strong>{{ $item->quantity }}</strong></span>
                                                                        <span class="fw-bold color-dorado-adaptativo">${{ number_format($item->price * $item->quantity, 0, ',', '.') }}</span>
                                                                    </div>
                                                                </div>
                                                            </div>
                                        ...
                                        <div class="mt-4 p-3 rounded border border-ui-adaptativa bg-superficie-adaptativa">
                                            <div class="row g-3 small">
                                                <div class="col-md-6">
                                                    <span class="text-muted-adaptativo d-block text-uppercase fw-bold mb-1" style="font-size: 0.75rem;">Datos de Entrega</span>
                                                    <p class="mb-1 color-adaptativo"><strong>Destinatario:</strong> {{ $order->customer_name }} {{ $order->customer_lastname }}</p>
                                                    <p class="mb-0 color-adaptativo"><strong>Dirección:</strong> {{ $order->delivery_street }} (CP {{ $order->delivery_postal_code }})</p>
                                                </div>
                                                <div class="col-md-6">
                                                    <span class="text-muted-adaptativo d-block text-uppercase fw-bold mb-1" style="font-size: 0.75rem;">Pago y Contacto</span>
                                                    <p class="mb-1 color-adaptativo"><strong>Email:</strong> {{ $order->customer_email }}</p>
                                                    <p class="mb-0 color-adaptativo"><strong>Método de pago:</strong> {{ $order->payment_method === 'transfer' ? 'Transferencia Bancaria' : 'Tarjeta de Crédito/Débito' }}</p>
                                                </div>
                                            </div>
                                        </div>
